<template>
  <div :class="['chat-session-item', currentSession ? 'active' : '']">
    <div class="contact-tag" v-if="data.conversationType !=1 "> 群 </div>
    <AvatarBase :userId="data.contactId"></AvatarBase>
    <div class="user-info">
      <div class="user-name-panel">
        <div class="user-name"> {{ data.conversationName }} </div>
        <div class="message-time"> {{ proxy.Utils.formatDate(data.lastMessageTime) }} </div>
      </div>
